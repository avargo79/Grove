...
///
///     GITGUI_SCREENSHOT=docs/screenshot.png GITGUI_SCREENSHOT_REPO=/path/to/repo dotnet test
/// </summary>
public class ScreenshotGenerator
{
    ...
    {
        var output = Environment.GetEnvironmentVariable("GITGUI_SCREENSHOT");
        if (string.IsNullOrWhiteSpace(output))
            return;

        var repositoryPath = Environment.GetEnvironmentVariable("GITGUI_SCREENSHOT_REPO")
                             ?? Directory.GetCurrentDirectory();

        // GITGUI_SCREENSHOT_THEME=light captures the light palette.
        if (Environment.GetEnvironmentVariable("GITGUI_SCREENSHOT_THEME") == "light")
            App.ApplyTheme(GitGui.Core.AppTheme.Light);
        ...
        var view = Environment.GetEnvironmentVariable("GITGUI_SCREENSHOT_VIEW");
        ...
        switch (view)
        {
            ...
            default:
                throw new InvalidOperationException(
                    $"GITGUI_SCREENSHOT_VIEW={view} is not a view; use 'working' or 'sidebyside'.");
        }
    ...
    ///
    ///     GITGUI_SCREENSHOT_CONFLICT=docs/screenshot-conflict.png dotnet test tests/GitGui.App.Tests
    /// </summary>
    [AvaloniaFact]
    public async Task WriteConflictScreenshot()
    {
        var output = Environment.GetEnvironmentVariable("GITGUI_SCREENSHOT_CONFLICT");
        if (string.IsNullOrWhiteSpace(output))
            return;

        using var fixture = GitGui.Core.Tests.TestRepository.CreateEmpty();
