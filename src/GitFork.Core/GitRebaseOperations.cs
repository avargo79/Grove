...
public sealed class GitRebaseOperations(GitCommandRunner git)
{
        ...
        string upstream, IReadOnlyList<RebaseTodoItem> todo, CancellationToken ct = default)
    {
        ...
        var directory = Directory.CreateTempSubdirectory("gitfork-rebase");
        try
        {
            ...
            var editor = await WriteSequenceEditorAsync(directory.FullName, todoPath, ct).ConfigureAwait(false);
    ...
    /// <summary>
    /// Writes a script that replaces git's plan file with ours. Git invokes the sequence editor
    /// with that file's path as its only argument.
    /// </summary>
    private static async Task<string> WriteSequenceEditorAsync(
        string directory, string todoPath, CancellationToken ct)
    {
        if (OperatingSystem.IsWindows())
        {
            var batch = Path.Combine(directory, "install-todo.cmd");
            await File.WriteAllTextAsync(batch, $"@copy /Y \"{todoPath}\" %1 >NUL\r\n", ct).ConfigureAwait(false);
            return batch.Replace('\\', '/');
        }

        var script = Path.Combine(directory, "install-todo.sh");
